...
function ohifInteractorStyleSlice(publicAPI, model) {
    ...
    publicAPI.handleMouseMove = (callData) => {
        ...
        switch (model.state) {
            ...
            case States.IS_DOLLY:
                ...
                let sliceActor = publicAPI.findSlice();
                if (sliceActor) {
                    const imageData = sliceActor.getMapper().getInputData();
                    publicAPI.invokeInteractionEvent({
                        type: 'InteractionEvent', dollyData: {
                            viewDirection,
                            imageData,
                            renderer,
                            sliceActor,
    ...
    publicAPI.moveSliceByWheel = (increment)=>{
        ...
        if (slice) {
            ...
            publicAPI.invokeInteractionEvent({type: 'InteractionEvent',wheelData: {viewDirection: viewDirection, displaySet: displaySet,sliceIndex: idx,sliceCount: idxCount,window: w,level: l}});
    ...
    publicAPI.windowLevel = (renderer, position) => {
        ...
        if (model.currentImageProperty) {
            ...
            let dx = (model.windowLevelCurrentPosition[0] - model.windowLevelStartPosition[0]) / size[0];
            let dy = (model.windowLevelStartPosition[1] - model.windowLevelCurrentPosition[1]) / size[1];
            ...
            let newLevel = level - dy;


            const slice = publicAPI.findSlice();
            const image = slice.getMapper().getInputData();

            const range = image.getPointData().getScalars().getRange();

            // we limit the window to the positive distance between low and high
            const distance = Math.abs(range[1] - range[0]);
            if (newWindow > distance){
                newWindow = distance;
            }

            // we limit the level to within the window.
            if (newLevel < 0) {
                newLevel = 0;
            }
            else if (newLevel > distance) {
                newLevel = distance;
            }
            ...
            let viewDirection = publicAPI.getViewDirection();
            let displaySet = publicAPI.getDisplaySet();
            publicAPI.invokeInteractionEvent({type: 'InteractionEvent',windowLevelData: {viewDirection: viewDirection, displaySet: displaySet,window: newWindow,level: newLevel}});
